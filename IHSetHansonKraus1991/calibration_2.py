...
class cal_Hybrid_2(CoastlineModel):
    ...
    def setup_forcing(self):
        ...
        if self.switch_Kal == 0:
            ...
            if self.cs_model == 'Yates et al. (2009)':
                self.nn = self.ntrs * 5 + (self.ntrs + 1)
                self.idx_list = [list(range(0, self.ntrs)),
                                 list(range(self.ntrs, 2*self.ntrs)),
                                 list(range(2*self.ntrs, 3*self.ntrs)),
                                 list(range(3*self.ntrs, 4*self.ntrs)),
                                 list(range(4*self.ntrs, 5*self.ntrs+1)),
                                 list(range(5*self.ntrs+1, 6*self.ntrs+1))]
            elif self.cs_model == 'Davidson et al. (2013)':
                self.nn = self.ntrs * 4 + (self.ntrs + 1)
                self.idx_list = [list(range(0, self.ntrs)),
                                 list(range(self.ntrs, 2*self.ntrs)),
                                 list(range(2*self.ntrs, 3*self.ntrs)),
                                 list(range(3*self.ntrs, 4*self.ntrs)),
    ...
    def init_par(self, population_size: int):
        ...
        if self.is_exp:
            lowers = np.hstack((lowers,np.log(np.array(self.lb[4]))))
            uppers = np.hstack((uppers,np.log(np.array(self.ub[4]))))
        else:
            lowers = np.hstack((lowers,np.array(self.lb)))
            uppers = np.hstack((uppers,np.array(self.ub)))
        
        # Adding vlt
        lowers = np.hstack((lowers, np.array([self.lb[5]])))
        uppers = np.hstack((uppers, np.array([self.ub[5]])))
